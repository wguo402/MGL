...
train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=96, shuffle=True,  num_workers=4)
# test_loader = torch.utils.data.DataLoader(dataset=test_dataset,  batch_size=48,  shuffle=False, num_workers=2)#48
val_loader = torch.utils.data.DataLoader(dataset=val_dataset,  batch_size=96,  shuffle=False, num_workers=4)#48
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train','test']:
            ...
            if phase == 'test' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
        torch.save(model.state_dict(), 'hr_food.pth')
        print()
    ...
    torch.save(model.state_dict(), 'hr_food_food.pth')
    return model
...
# pretrainde_dict = (torch.load('hrnetv2_w64_imagenet_pretrained.pth'))
# pretrainde_dict = (torch.load('hr_food_food.pth'))
# # pretrainde_dict = (torch.load('hr_food_100.pth'))
...
model_ft = nn.DataParallel(model_ft)
# # model_ft.load_state_dict(pretrainde_dict)
