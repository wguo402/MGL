...
if __name__ ==  '__main__':
    ...
    data_txt = open(os.path.join('/home1/food/VireoFood172/SplitAndIngreLabel', data_txt_dir))
    data_line = data_txt.readline()

    data_txt_test = open(os.path.join('/home1/food/VireoFood172/SplitAndIngreLabel', data_test_txt_dir))
    data_line_test = data_txt_test.readline()

    train_output_line=[]
    test_output_line = []
    dict_key = random.sample(label_dict.keys(), 40)
    test_dict={}
    for i,j in enumerate(dict_key):
         test_dict[j]=i
         print(label_dict[j])
         del label_dict[j]
    dict_key = random.sample(label_dict.keys(), 132)
    train_dict={}
    for i,j in enumerate(dict_key):
        train_dict[j] = i
        print(label_dict[j])
    ...
    while data_line_test:
        temp_line = str(int(data_line_test[0:data_line_test.rfind('/', 1)])-1)
    ...
    while data_line:
        temp_line = str(int(data_line[0:data_line.rfind('/', 1)])-1)
    ...
    with open(os.path.join('/home1/food/VireoFood172/SplitAndIngreLabel', output_file_test),'w') as f:
