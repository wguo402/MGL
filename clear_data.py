...
class MyDataset(data.Dataset):
    ...
    def __init__(self, txt_dir,save_path):
        self.image_path ='/home1/food/isiaFood_200/'
    ...
    def __getitem__(self, index):
        ...
        try:
            img = self.loader(src_path)
            output_text = (img_name+' '+str(label))
            print(index)
            # temp_line = img_name[0:img_name.rfind('/', 1)]
            # dst_path_temp = self.save_path + '/' + temp_line
...
for i,j in enumerate(generate_train_dataset):
    if j :
        output.append(j)
with open(os.path.join('/home1/food/isiaFood_200/pytorch', 'train_clear.txt'),'w') as f:
    for j in output:
        f.write(str(j)+'\n')
        pass
...
for i,j in enumerate(generate_val_dataset):
    if j:
        output.append(j)
with open(os.path.join('/home1/food/isiaFood_200/pytorch', 'val_clear.txt'), 'w') as f:
    for j in output:
        f.write(str(j) + '\n')
        pass
output=[]
for i,j in enumerate(generate_test_ataset):
    if j:
        output.append(j)
with open(os.path.join('/home1/food/isiaFood_200/pytorch', 'test_clear.txt'), 'w') as f:
    for j in output:
        f.write(str(j) + '\n')
        pass
    pass
